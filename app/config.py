...
class Settings(BaseSettings):
    ...
    memory_store_dir: str = Field(default="stores/memory")  # ⭐ 记忆库目录
    ...
    memory_token_budget: int = 800  # ⭐ 记忆拼接预算（approx tokens）
...
for _p in [
    settings.data_pdf_dir,
    settings.model_dir,
    settings.faiss_store_dir,
    settings.bm25_store_dir,
    settings.memory_store_dir,  # ⭐ memory 目录
]:
